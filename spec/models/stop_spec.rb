...
describe Stop, type: :model do
  ...
  describe ".ci_search" do
    it "returns no results if the name is blank" do
      FactoryGirl.create :stop, name: "Fullerton"
      test_subject = nil
      expect(described_class.ci_search(name: test_subject)).to be_nil
    end

    it "returns no results if the name is empty" do
      FactoryGirl.create :stop, name: "Howard"
      test_subject = ""
      expect(described_class.ci_search(name: test_subject)).to be_nil
    end

    it "returns true if the stop name matches an available stop" do
      FactoryGirl.create :stop, name: "Thorndale"
    ...
    it "returns true if the stop name matches an available stop, regardless of case" do
      FactoryGirl.create :stop, name: "Midway"
      test_subject = "miDWaY"
      expect(described_class.ci_search(name: test_subject)).not_to be_nil
    end

    it "returns false if the stop name does not match an available stop" do
      FactoryGirl.create :stop, name: "O'Hare"
      test_subject = "ouch"
      expect(described_class.ci_search(name: test_subject)).to be_nil
    end
  end
  ...
  describe ".stop_id_valid?" do
    ...
    it "returns false if the external_id does not match an available submission" do
      FactoryGirl.create :stop, name: "Belmont", external_id: 1234
      test_subject = 3425
      expect(described_class.stopid_valid?(external_id: test_subject)).to be_falsey
    end
  end
