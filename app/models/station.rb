  ...
  def self.render_ssml(slot:)
    if slot[:name] == Slots::STATION_ID
      if !slot[:present]
        ...
        "<speak>Sorry, I don't know that station i.d.<break time='1s'/>Station numbers are five digits long and start with the number 3.<break time='1s'/>Please say a valid station i.d.</speak>"
      else
